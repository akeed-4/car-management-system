...
export class CreditSaleListComponent implements OnInit {
  ...
  loadInvoices(): void {
    ...
    this.salesService.getInvoices().subscribe({
      next: invoices => {
        this.invoices.set(invoices.filter(i => i.saleType === SaleType.Credit && (i.salesChannel === undefined || i.salesChannel === SalesChannel.Afrad)));
        this.loading.set(false);
      },
      error: () => {
        this.loading.set(false);
        this.notificationService.showError('DIRECT_SALES.CREDIT.LOAD_FAILED');
      }
    });
  }
  ...
  onCreateNew(): void {
    this.router.navigate(['/sales/direct/credit-sale/new']);
  }
        ...
        exportDataGrid({ component, worksheet }).then(() => {
          workbook.xlsx.writeBuffer().then((buffer: BlobPart) => {
            import('file-saver').then(({ saveAs }) => {
              saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'CreditSales.xlsx');
            });
          });
        });
      });
