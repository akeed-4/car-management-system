...
export class SalesComponent {
  ...
  archiveInvoice(e: any) {
    this.salesService.archiveInvoice(e.row.data.id);
  }

  // Fix: Used salesService.unarchiveInvoice
  unarchiveInvoice(e: any) {
    this.salesService.unarchiveInvoice(e.row.data.id);
  }

  deleteInvoice(e: any) {
    const id = e.row.data.id;
    if (confirm(this.translate.instant('SALES.DELETE_INVOICE_CONFIRM'))) {
      this.salesService.deleteInvoice(id).subscribe({
        next: () => {
          // Refresh the invoices list
          this.invoices = toSignal(this.salesService.getInvoices(), { initialValue: [] });
        },
        error: (error) => {
          console.error('Error deleting invoice:', error);
          alert(this.translate.instant('SALES.DELETE_INVOICE_ERROR'));
        }
      });
    }
  }
  ...
  printInvoice(e: any) {
    this.router.navigate(['/sales/invoice/print', e.row.data.id]);
  }

  editInvoice(e: any) {
    this.router.navigate(['/sales/invoice/edit', e.row.data.id]);
  }
